...
vertice3 = Vertex('c')
print(vertice1)
vertice1.add_neighbor(vertice2, 10)
print(vertice1)
vertice1.add_neighbor(vertice3, 20)
print(vertice1.get_connections())
print(vertice1)
'''
...
class Graph:
    ...
    def add_edge(self, frm, to, cost = 0):  # frm and to should be str id's
...
def dijkstra(aGraph, start):
    ...
    while len(unvisited_queue):
        ...
        current.set_visited() # add node to set N'
        ...
        for next in current.adjacent:
            ...
            if new_dist < prev_dist:
                ...
                next.set_previous(current) # the previous node in the shortest path to the source               
        ...
        unvisited_queue = [(v.get_distance(), id(v), v) for v in aGraph if v.visited == False]
        heapq.heapify(unvisited_queue)
...
if __name__ == '__main__':
    ...
    # print(labels)    
    position = nx.spring_layout(G) # A dictionary with nodes as keys and positions as values
    # print(position)    
    # nx.draw_networkx_edges(G, position, SPT_edges)

    nx.draw_networkx(G, position, edgelist = SPT_edges, width = 3.0, edge_color = 'red', with_labels=True)
    nx.draw_networkx(G, position, with_labels=True)
    nx.draw_networkx_edge_labels(G, position, edge_labels = labels, font_size = 12, font_color = 'blue')
